...
// Phase1: ホーム新版 - 視点 (booking/visit/contract) と 分析軸 (facility/treatment/promo)
let _homeAnalysisView = (() => {
  try { return sessionStorage.getItem('home-analysis-view') || 'booking'; } catch(_) { return 'booking'; }
})();
let _homeAnalysisDim = (() => {
  try { return sessionStorage.getItem('home-analysis-dim') || 'facility'; } catch(_) { return 'facility'; }
})();
function _saveHomeView()  { try { sessionStorage.setItem('home-analysis-view', _homeAnalysisView); } catch(_){} }
function _saveHomeDim()   { try { sessionStorage.setItem('home-analysis-dim', _homeAnalysisDim); } catch(_){} }
let _homeAnalysisSort = (() => {
  try { const s = sessionStorage.getItem('home-analysis-sort'); return s ? JSON.parse(s) : { key: 'booking', dir: 'desc' }; }
  catch(_) { return { key: 'booking', dir: 'desc' }; }
})();
...
function renderHomeDashboard() {
  ...
  const _amountOf = (d) => {
    const ex = _bkExtraHome[d.name + '|' + d.applyDate] || {};
    return Number(ex.contractAmount) || Number(d.contractAmount) || 0;
  };
  // 共通: 軸ごとの集計関数 (現期間 + 前期間で再利用)
  const aggregateBy = (rows, keyFn) => {
    const map = {};
    rows.forEach(d => {
      const k = keyFn(d) || '-';
      if (!map[k]) map[k] = { booking: 0, visited: 0, contracted: 0, amount: 0 };
      map[k].booking++;
      if (isVisited(d.status)) map[k].visited++;
      if (d.status === '成約') {
        map[k].contracted++;
        map[k].amount += _amountOf(d);
      }
    });
    return map;
  };
  const facKey = (d) => normFac(d.facility) || '-';
  const treatKey = (d) => (typeof normSvc === 'function' ? normSvc(d.service) : d.service) || '-';
  const promoKey = (d) => (d.source && d.source.trim() && d.source.trim() !== '?') ? d.source : '(なし)';
  const byFacMonth = aggregateBy(rangeRows, facKey);
  const byTreatMonth = aggregateBy(rangeRows, treatKey);
  const byPromoMonth = aggregateBy(rangeRows, promoKey);
  ...
  el.innerHTML = `
    ...
    <!-- Phase1: ホーム新版 - 視点 × 軸 トグル分析テーブル (旧: 医院別/治療別/プロモmatrixカード) -->
    ${facList.length > 0 ? `
      <div style="margin-bottom:14px;padding:12px;background:var(--card);border:1px solid var(--border);border-radius:var(--radius-sm)">
        <div style="display:flex;align-items:center;flex-wrap:wrap;gap:6px;margin-bottom:10px">
          ...
          ${(() => {
            ...
            const btn = (v, l) => `<button class="home-dim-btn filter-btn filter-btn-dark ${dim===v?'is-active':''}" data-dim="${v}">${l}</button>`;
            return btn('facility', '🏥 医院別') + btn('treatment', '🦷 治療別') + btn('promo', '🎯 プロモ別');
          })()}
        </div>
          ...
          ${(() => {
            ...
            const metric = (k, key) => {
              ...
              switch (key) {
                ...
                case 'visited':    return v.visited;
                case 'visitRate':  return v.booking ? v.visited/v.booking : -1;
            ...
            // ソート適用
            const sort = _homeAnalysisSort;
            const list = [...baseList].sort((a,b) => {
              const va = metric(a, sort.key), vb = metric(b, sort.key);
              if (typeof va === 'string') return sort.dir==='asc' ? va.localeCompare(vb,'ja') : vb.localeCompare(va,'ja');
              return sort.dir==='asc' ? va - vb : vb - va;
            });
            // 合計行 (footer)
            const tot = baseList.reduce((s,k) => {
              const v = data[k] || { booking:0, visited:0, contracted:0, amount:0 };
  ...
  // Phase1: 視点トグル (予約/来院/成約)
  el.querySelectorAll('.home-view-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      _homeAnalysisView = btn.dataset.view;
      _saveHomeView();
      renderHomeDashboard();
      if (typeof renderAnalyticsDashboard === 'function') renderAnalyticsDashboard();
    });
  });
  // Phase1: 軸トグル (医院別/治療別/プロモ別)
  el.querySelectorAll('.home-dim-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      _homeAnalysisDim = btn.dataset.dim;
      _saveHomeDim();
      renderHomeDashboard();
      if (typeof renderAnalyticsDashboard === 'function') renderAnalyticsDashboard();
    });
  ...
  // Phase1: 列クリックでソート
  el.querySelectorAll('.home-sort-th').forEach(th => {
    th.addEventListener('click', () => {
      const key = th.dataset.sortkey;
      if (_homeAnalysisSort.key === key) {
        _homeAnalysisSort.dir = _homeAnalysisSort.dir === 'asc' ? 'desc' : 'asc';
      } else {
        _homeAnalysisSort = { key, dir: key === 'name' ? 'asc' : 'desc' };
      }
      _saveHomeSort();
      renderHomeDashboard();
      if (typeof renderAnalyticsDashboard === 'function') renderAnalyticsDashboard();
...
function renderBookings() {
  ...
  document.getElementById('bk-stats').innerHTML = `
    <div class="stat-card" title="予約数 = 来院済 + キャンセル + 未来予約 + 進行中 (申込総数なのでキャンセルも母数に含む / 要対応はキャンセル内)"><span class="stat-label">予約数</span><span class="stat-num">${total}</span><span class="stat-yoy" style="color:var(--text-sub);font-size:10px;font-weight:400">合計件数</span></div>
...
async function renderKaiinAll(containerId) {
  ...
  const _is見込み = (d) => !_is実成約(d) && _amt(d) > 0;
  const byCat = {};
  const byFac = {};
  allRows.forEach(d => {
  ...
  el.innerHTML = `
    ...
    <div class="stats-row" style="gap:6px;margin-bottom:8px">
      <div class="stat-card" title="予約数 = 来院済 + キャンセル + 進行中 (来店管理は未来予約を含まない / 申込総数なのでキャンセルも母数に含む)"><span class="stat-label">予約数</span><span class="stat-num">${totalCount}</span><span class="stat-yoy" style="color:var(--text-sub);font-size:10px;font-weight:400">合計件数</span></div>
...
function renderKaiinSimpleList(treatment, rows, containerId) {
  ...
  el.innerHTML = `
    ...
    <div class="stats-row kaiin-summary-row" style="gap:6px;margin-bottom:8px;display:none">
      <div class="stat-card" title="予約数 = 来院済 + キャンセル + 進行中 (来店管理は未来予約を含まない / 申込総数なのでキャンセルも母数に含む)"><span class="stat-label">予約数</span><span class="stat-num kaiin-sum-total">${sum.total}</span><span class="stat-yoy" style="color:var(--text-sub);font-size:10px;font-weight:400">合計件数</span></div>
...
function drawKaiinRows(treatment, rows, container) {
  ...
  (function updateSummary(){
    ...
    try {
      ...
      // v428: 見込み = 売上金額が入力されているが、まだ実成約ではない人
      const _is見込みDR = (d) => !_is実成約DR(d) && _amtDR(d) > 0;
...
function renderFollowup() {
        ...
        <table class="data-table compact" style="width:100%;font-size:12px">
          ...
          <tbody>
            ${visibleRows.map(c => {
              ...
              const baseRowBg = c.rebooking ? '#f0fdf4' : c.category === 'review' ? '#f0f9ff' : '';
  ...
  const updateFollowupSelectionUI = () => {
    const selectedCount = _followupState.selected.size;
    const bar = el.querySelector('#followup-bulk-bar');
    if (bar) bar.style.display = selectedCount > 0 ? 'flex' : 'none';
    const countEl = el.querySelector('#followup-selected-count');
    if (countEl) countEl.textContent = `✓ ${selectedCount}件 選択中`;

    const checks = Array.from(el.querySelectorAll('.followup-row-check'));
    let visibleSelected = 0;
    checks.forEach(cb => {
      const selected = _followupState.selected.has(cb.dataset.key);
      cb.checked = selected;
      if (selected) visibleSelected++;
      const row = cb.closest('.followup-row');
      if (row) row.style.background = selected ? '#ede9fe' : (row.dataset.baseBg || '');
    });

    const selectAll = el.querySelector('#followup-select-all');
    if (selectAll) {
      selectAll.checked = checks.length > 0 && visibleSelected === checks.length;
      selectAll.indeterminate = visibleSelected > 0 && visibleSelected < checks.length;
    }
...
function renderPromo() {
  ...
  el.querySelectorAll('.promo-hide-notice').forEach(cb => {
    cb.addEventListener('change', () => {
      _promoTabState.hideNoticeCreated = cb.checked;
      renderPromo();
    });
  });
  // 支払い通知書作成 (v383: 確認ダイアログ + 対象月)
  el.querySelectorAll('.promo-create-notice').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      _openPaymentNoticeModal(src, selectedBookings, _bkEx, targetMonth);
    });
  });
}

// === v380/383: 支払い通知書モーダル (月1バッチ印刷 + 確定で paymentNoticeCreated 保存) ===
function _openPaymentNoticeModal(source, bookings, _bkEx, targetMonth) {
  // 既存モーダルがあれば削除
  ...
  // v383: 対象月のラベル (ボタンから受け取った targetMonth='YYYY-MM' または現在月)
  const tm = targetMonth || `${issueDate.getFullYear()}-${String(issueDate.getMonth()+1).padStart(2,'0')}`;
  const tmParts = tm.split('-');
  const targetMonthLabel = `${tmParts[0]}年${parseInt(tmParts[1])}月分`;
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:10px;max-width:760px;width:100%;box-shadow:0 12px 40px rgba(0,0,0,.35);font-family:inherit">
      ...
      <div class="notice-actions" style="display:flex;align-items:center;gap:8px;padding:12px 16px;border-bottom:1px solid var(--border);background:#f9fafb;border-radius:10px 10px 0 0">
        <strong style="font-size:14px">📄 ${escapeHtml(targetMonthLabel)} 支払い通知書 - ${bookings.length}件 / ¥${fmt(totalInc)}</strong>
        <span style="margin-left:auto;display:flex;gap:6px">
          <button id="notice-print-btn" class="filter-btn" title="印刷">🖨 印刷</button>
          <button id="notice-confirm-btn" class="filter-btn" style="background:#16a34a;color:#fff;font-weight:600" title="この内容で確定 (確認ダイアログあり)">✓ 作成確定</button>
          <button id="notice-cancel-btn" class="filter-btn" title="閉じる (確定しない)">✕ 閉じる</button>
        </span>
      </div>
      <!-- 印刷対象 -->
      <div id="notice-printable" style="padding:30px 36px;font-family:'Hiragino Mincho ProN',serif;color:#111">
        ...
        <div style="text-align:center;font-size:14px;color:#7c3aed;font-weight:700;margin-bottom:8px">${escapeHtml(targetMonthLabel)}</div>
        ...
        <div style="font-size:11px;color:#666;margin-bottom:24px">下記の通り、${escapeHtml(targetMonthLabel)}の紹介報酬をお支払いいたします。</div>
  ...
  modal.querySelector('#notice-confirm-btn').addEventListener('click', async () => {
    // v383: 確認ダイアログ (取り消し不可の警告)
    const ok = window.confirm(
      `${targetMonthLabel} 支払い通知書を作成します。\n\n` +
      `宛先: ${source}\n` +
      `件数: ${bookings.length}件\n` +
      `合計: ¥${fmt(totalInc)}\n\n` +
      `※ 確定後、これらの案件は通知書作成済みとなり、再発行できなくなります。\n` +
      `本当によろしいですか?`
    );
    if (!ok) return;
    // 全件に paymentNoticeCreated=true + paymentNoticeDate + paymentNoticeMonth 保存
    try {
      const bkEx = loadData('bk-extra', {});
      const updates = [];
      for (const d of bookings) {
        ...
        bkEx[key].paymentNoticeMonth = tm;  // v383: '2026-05' バッチ識別用
      ...
      _promoTabState.selected.clear();
      modal.remove();
      showToast(`✓ ${targetMonthLabel} 通知書を ${bookings.length}件 作成しました (¥${fmt(totalInc)})`);
      renderPromo();
      if (typeof syncCrossTabRender === 'function') syncCrossTabRender();
    } catch (e) {
...
function _renderPromoDetail(p, _bkEx) {
  ...
  // 成約のみ表示 (インセ管理対象)
  // v382/383: hideZeroIncentive / hidePaid / hideNoticeCreated でフィルタ
  let contracted = p.bookings.filter(b => b.status === '成約');
  const totalContracted = contracted.length;
  let hiddenZero = 0, hiddenPaid = 0, hiddenNotice = 0;
  ...
  if (_promoTabState.hideNoticeCreated) {
    contracted = contracted.filter(b => {
      if (_noticeCreated(b)) { hiddenNotice++; return false; }
      return true;
    });
  }
  contracted.sort((a, b) => {
  ...
  // v383: 対象月 = 現在の年月 (バッチ発行用)
  const _now = new Date();
  const _targetMonth = `${_now.getFullYear()}-${String(_now.getMonth()+1).padStart(2,'0')}`;
  ...
  return `<div class="promo-detail-section" data-source="${escapeHtml(p.source)}" style="padding:8px">
    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:6px">
      ...
      <span style="margin-left:auto;display:flex;align-items:center;gap:8px;flex-wrap:wrap">
        ...
        <label style="font-size:11px;display:inline-flex;align-items:center;gap:3px;cursor:pointer;user-select:none" title="既に支払い済みの案件を非表示にする (二重支払い防止)">
          <input type="checkbox" class="promo-hide-paid" ${_promoTabState.hidePaid?'checked':''} style="cursor:pointer">
          <span>支給済</span>
        </label>
        <label style="font-size:11px;display:inline-flex;align-items:center;gap:3px;cursor:pointer;user-select:none" title="通知書作成済の案件を非表示にする (月1回まとめ発行・二重通知防止)">
          <input type="checkbox" class="promo-hide-notice" ${_promoTabState.hideNoticeCreated?'checked':''} style="cursor:pointer">
          <span>通知済</span>
        </label>
    ...
    <table class="data-table compact" style="width:100%;font-size:11px;margin-bottom:10px">
      ...
      <tbody>
        ${contracted.map(d => {
          ...
          const fmtMD = (s) => { const m = String(s||'').match(/(\d{4})\D+(\d{1,2})\D+(\d{1,2})/); return m ? `${parseInt(m[2])}/${parseInt(m[3])}` : '－'; };
          // 通知書セルの表示内容 (v383: 対象月もラベル表示)
          const noticeMonth = ((_bkEx[d.name + '|' + d.applyDate] || {}).paymentNoticeMonth) || d.paymentNoticeMonth || '';
          const noticeMonthLbl = noticeMonth ? noticeMonth.replace(/^\d{2}(\d{2})-(\d{2})$/, '$1/$2') : '';
          const noticeCell = noticeCreated
            ? `<span style="display:inline-block;padding:2px 6px;background:#dcfce7;color:#15803d;border-radius:10px;font-size:10px;font-weight:600;border:1px solid #86efac" title="発行日: ${escapeHtml(noticeDate)} / 対象月: ${escapeHtml(noticeMonth)}">📄 ${escapeHtml(noticeMonthLbl||'作成済')}</span>`
